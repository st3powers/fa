...
#=============================================================================
# ----> read in secchi data
# North Temperate Lakes LTER: Secchi Disk Depth; 
# Other Auxiliary Base Crew Sample Data 1981 - current
...
seasons_lakes_wide <- seasons_lakes %>%
  melt(id.vars = c("lakeid", "year", "season"), variable.name = "category",
       value.name = "date") %>%
  ...
  select(lakeid, winter_yr, iceon_startdate, iceon_enddate, iceoff_startdate,
         iceoff_enddate)
...
secchi_seasons_pre <- merge(secchi_dates_only, seasons_lakes_wide,
  ...
  # tag seasons
  mutate(season = ifelse(date >= iceon_startdate & date <= iceon_enddate,
                         "iceon", NA),
         season = ifelse(date >= iceoff_startdate & date <= iceoff_enddate,
                         "iceoff", season)) %>%
...
# filter(secchi_seasons, season == "iceon" & is.na(ice)) %>%
#   filter(lakeid == "ME")
...
madison_fa <- read.csv("../Data/LTER_Madison_weighted_year_season_FA.csv",
...
icesnow <- icesnow_orig %>% select(lakeid, year = year4, sampledate, sta, avsnow,
                                 sdsnow, totice, whiteice, blueice)
...
light <- light_orig %>%
  select(lakeid, year = year4, sampledate, sta, depth, light)
...
chlr_north <- chl_north_orig %>% select(lakeid, year = year4, sampledate,
                                      depth, chl = chlor, phaeo)
chlr_south <- chl_south_orig %>% select(lakeid, year = year4, sampledate,
                                      depth = depth_range_m, chl = tri_chl_spec,
                                      phaeo = phaeo_spec)
...
merge <- merge(icesnow, chlr, by = c("lakeid",
                                     "sampledate",
                                     "year")) #keep only dates that are in both
snowicechl <- merge
...
merge <- merge(snowicechl, light, by = c("lakeid", "sampledate", "depthgroup",
                                        "sta", "year"))
...
snowicechllight.aggdepths <- snowicechllight %>%
  group_by(lakeid, sampledate, avsnow, sdsnow, totice, whiteice, blueice) %>%
  dplyr::summarize(chl = mean(chl, na.rm = TRUE),
                   light = mean(light, na.rm = TRUE)) %>%
  as.data.frame()

snowicechllight.aggdepthgroups <- snowicechllight %>%
  group_by(lakeid, depthgroup, sampledate, avsnow,
           sdsnow, totice, whiteice, blueice) %>%
  dplyr::summarize(chl = mean(chl, na.rm = TRUE),
                   light = mean(light, na.rm = TRUE)) %>%
  as.data.frame()
...
lightsnow_BM <- ggplot(subset(snowicechllight.shallow,
                              snowicechllight.shallow$lakeid == "BM"),
                       aes(x = avsnow, y = light)) +
...
png(filename = "../Figures/lightsnow_7lakes.png", width = 4, height = 4,
    units = "in", res = 500)
lightsnow_7lakes
dev.off()


png(filename = "../Figures/lightchla_BM.png", width = 2.5, height = 2.5,
    units = "in", res = 500)
lightchla_BM
dev.off()

png(filename = "../Figures/lightsnow_BM.png", width = 2.5, height = 2.5,
    units = "in", res = 500)
lightsnow_BM
dev.off()